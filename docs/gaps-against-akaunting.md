...
That is the whole of it — one item, since `0527` closed the other. This
list carried **Configurable dashboards** as the second of two for as
long as section 10 did, and section 10 was wrong about it. This document has repeatedly been left saying
...
- ~~**Configurable dashboards and widgets.** `Common/Dashboard.php`,
  ...
  iAkauntan's dashboard is fixed~~ — **half closed at `0527`**, and the
  half that closed is the half this section predicted.

  What it argued: the dashboard is already filtered by what a company
  bought (`0137`) and by what a person may reach (`0127`), which is
  most of what arranging it by hand is for — a warehouse clerk does not
  see payroll and a company without POS does not see takings. What was
  left was ORDERING and HIDING, and the cost of both together is a
  widget registry, a layout per user, a drag-and-drop editor and a
  migration every time a card changes shape. So the section ended: "if
  somebody does, the thing to build is probably 'hide this card', not a
  layout engine."

  That is what `0527` built. `user_preferences.dashboard_cards` is a
  `text[]` per PERSON rather than per company; `Settings > Landing
  page` draws a switch per card under "Turn off what you do not read";
  and the dashboard asks `cards.contains(...)` before drawing each of
  the five. A person who has never opened that screen gets the column
  default, which `supabase/tests/user_preferences.sql` and
  `app/test/landing_preference_test.dart` hold against the Dart
  constant, so a bare dashboard is not reachable by doing nothing.

  ORDERING is still not offered, and deliberately: the stored list is
  read with `contains`, so its order has no effect, and the editor
  writes it back in the catalogue's order rather than in the order the
  switches were touched — otherwise the dashboard would rearrange
  itself as somebody toggled. The argument above is the reason, and it
  is unchanged. What was wrong was only the claim that the dashboard is
  fixed.

  The e-Invoice banner is deliberately not one of the five. It is a
  REFUSAL waiting to be dealt with rather than a panel, and a rejected
  submission somebody has switched off is one LHDN is still waiting
  for.
