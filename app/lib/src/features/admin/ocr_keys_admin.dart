...
// `RepoOcrKeys` is an extension, and a Dart extension is only in scope
...
class _OcrKeysAdminTabState extends ConsumerState<OcrKeysAdminTab> {
        ...
        return Scaffold(
          ...
          body: ListView(
            ...
            children: [
              ...
              // pool holds, and somebody filling a pool to fix a
              // stopped scan needs to know that is not the problem.
  ...
  Future<void> _edit(String provider, OcrPoolKey? existing) async {
    final saved = await showDialog<bool>(
      context: context,
      builder: (_) => _KeyDialog(provider: provider, existing: existing),
    );
    if (saved == true) {
      ref.invalidate(ocrKeyPoolProvider((provider: provider, orgId: null)));
    }
  }
}

/// One reader's pool, drawn.
class _Pool extends ConsumerWidget {
  const _Pool({required this.provider});

  final String provider;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final args = (provider: provider, orgId: null as String?);
      ...
      builder: (keys) {
        if (keys.isEmpty) {
          return const Padding(
            ...
            child: EmptyState(
              icon: Icons.vpn_key_outlined,
              title: 'No keys yet',
              message: 'A reader with an empty pool falls back on the key '
                  'in the function’s environment, and fails when '
                  'there is none. Add one and it is used from the next '
                  'scan.',
            ),
          );
        }
        ...
        return Column(
          ...
          children: [
            ...
            for (final k in keys) _KeyRow(provider: provider, poolKey: k),
          ],
        );
      },
    );
...
class _KeyRow extends ConsumerWidget {
  const _KeyRow({required this.provider, required this.poolKey});
  ...
  Future<void> _edit(BuildContext context, WidgetRef ref) async {
    final saved = await showDialog<bool>(
      context: context,
      builder: (_) => _KeyDialog(provider: provider, existing: poolKey),
    );
    if (saved == true) {
      ref.invalidate(ocrKeyPoolProvider((provider: provider, orgId: null)));
    }
  ...
  Future<void> _remove(BuildContext context, WidgetRef ref) async {
    ...
    await runWithFeedback(
      context,
      action: () =>
          ref.read(ocrKeyPoolApiProvider).remove(provider, poolKey.id),
...
class _KeyDialog extends ConsumerStatefulWidget {
  const _KeyDialog({required this.provider, required this.existing});
