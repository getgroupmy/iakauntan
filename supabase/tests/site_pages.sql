-- =====================================================================
-- iAkauntan :: the five pages around the product
--
...
-- ---------------------------------------------------------------------
-- Five rows, and the table will not hold a sixth kind
--
-- The slugs are fixed because five screens read them. A row nothing
...
begin
  ...
  perform pg_temp.check_eq('the five pages are there to open', v_n, 5);

  select count(*) into v_n from public.site_pages
   where slug in ('signin', 'signup', 'terms', 'privacy', 'contact');
  perform pg_temp.check_eq('and they are the five the screens read', v_n, 5);

  begin
    insert into public.site_pages (slug) values ('about');
  exception when check_violation then v_refused := true;
  end;
  perform pg_temp.check_true('a sixth slug is refused by the table', v_refused);
end $$;
...
-- ---------------------------------------------------------------------
-- A mistyped slug is told which five there are
--
...
begin
  ...
  perform pg_temp.check_true('and the refusal names the five',
    v_msg like '%signin%' and v_msg like '%contact%');
end $$;
