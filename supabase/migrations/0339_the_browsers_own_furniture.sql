-- ---------------------------------------------------------------------
-- 0339 - the browser's own furniture
--
-- `web/index.html` and `web/manifest.json` carry a theme colour, a
-- title and a description in plain text. Every deployment of this
-- software therefore tells the browser it is iAkauntan: the tint on an
-- Android address bar, the name on a tab and on an iOS home screen, the
-- sentence a link preview shows, and the whole of what a PWA install
-- reads.
--
-- Those are files inside the bundle, so two things have to happen and
-- neither is enough alone. CI stamps them from `landing_page()` before
-- the build — the only moment a file in the bundle can change, and the
-- only thing an install prompt ever sees. And the running page repoints
-- the same tags once the payload arrives, so a colour changed in the
-- console reaches a visitor on the next load rather than the next
-- deploy. Exactly the arrangement the favicon has had since `0314`.
--
-- ## No new columns
--
-- `brand_colour` already exists. So do `meta_title` and
-- `meta_description` — since `0290`, editable in the console since
-- `0316`, and read by nothing whatsoever until now. An operator has
-- been able to write a browser-tab title for four migrations and watch
-- it change nothing.
--
-- All this migration does is put the two meta fields in `brand` beside
-- the colour, which is ungated: a tab has a title on every screen,
-- including every screen a platform with no published marketing site
-- still shows.
-- ---------------------------------------------------------------------
