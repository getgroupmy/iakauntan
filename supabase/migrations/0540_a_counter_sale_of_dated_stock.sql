-- =====================================================================
-- 0540. A counter sale of dated stock
--
-- A pharmacy sells the box of tablets itself; a mini-market sells the
-- carton of milk itself. Neither is a recipe, and neither could be rung
-- through the till: `complete_pos_sale` came back with
--
--     Item UBAT is tracked by batch, so every unit has to be named
--     before this can be posted.
--
-- raised by `app.materialise_movement_lots` off the delivery movement
-- the counter invoice posts. A tracked item reaching the counter as an
-- INGREDIENT worked, because that movement carries
-- `source_table = 'pos_sales'` and reaches the earliest-expiry-first
-- arm. The item ITSELF did not: its movement is the invoice's, and
-- `sales_documents` is deliberately kept out of that arm.
--
-- Two ways to fix it, and only one of them is right.
--
-- Putting `sales_documents` in the list would quietly start picking
-- batches for EVERY ordinary invoice line somebody forgot to name lots
-- on. That refusal is not an oversight: an office typing an invoice can
-- be asked which batch went, and a silent pick there is how a recall
-- misses the customer holding the affected box.
--
-- So the POS answers for itself. It picks earliest expiry first at the
-- moment the sale completes and writes the answer onto the invoice line
-- as `document_line_lots`, which is the same place a typed invoice
-- keeps it. The invariant is untouched; the till simply stops being
-- silent. FEFO is also what the shop does with its hands.
--
-- And a shelf that cannot cover the line is refused here, in words a
-- cashier can act on, rather than posting an invoice whose named
-- batches do not add up to the units that left.
--
-- BATCH ONLY, AND SERIAL DELIBERATELY NOT. A batch names a production
-- run, and which carton of the run the customer carried out is not a
-- fact anybody at a counter knows. A serial names one physical unit:
-- picking one silently would print a serial on the customer's warranty
-- for a machine somebody else is holding, and the first anyone would
-- hear of it is a claim being refused. That wants a scan field at the
-- till, so a serialised item is still refused here -- but in words that
-- say what is missing rather than in the invariant's.
--
-- `supabase/tests/pos_tracked_item_sale.sql` sells three of a
-- batch-tracked item with two on the early batch and five on the late
-- one, and asks which batch left.
-- =====================================================================
...
begin
  ...
  loop
    ...
    -- BATCH AND NOT SERIAL, and the difference is not a detail.
    --
    ...
    -- needs a scan field at the till, not a guess here, so it stays
    -- refused -- in words that say what is missing.
    if coalesce(v_kept, false) and coalesce(v_track, 'none') = 'serial' then
      raise exception
        '% is tracked by serial number, and a till has nowhere to scan '
        'one. Invoice it from Sales, where the unit sold can be named.',
        v_line.description using errcode = '23514';
    end if;
