-- A month of unpaid leave reported its statutory figures as checked.
--
-- `app.calc_statutory` has a branch written for exactly this, and its
-- own comment says why it is there:
--
--   "A month of unpaid leave. There is nothing to contribute on, but
--    the schedule is whatever it is -- reporting `false` here printed
--    'the statutory figures on this payslip have not been verified' on
--    a payslip whose schedules were fine."
--
-- and, on the band it cannot find:
--
...
--    -- and `payslip_pdf.dart` prints its warning off that flag, so the
--    one payslip that most needed a warning was the one that got none."
--
-- `calculate_payroll_run` never reached either branch. It guarded all
-- three calls with `and v_base.X_wage > 0`, so on a zero wage the
-- function was not called at all -- and the caller's own variables kept
-- the values they were reset to a few lines above: schedule id null,
-- `v_ver` TRUE.
--
-- Measured, not argued. One employee, a full month of approved unpaid
-- leave, and an EPF table published and deliberately NOT marked
-- verified:
--
--   epf_wage=0.00  epf_ee=0.00  epf_schedule_id=NULL  VERIFIED=t
--
-- The payslip says the statutory figures were checked. The EPF table
-- behind it was not. That is the same bug `0409` fixed inside
-- `calc_statutory`, arriving from the caller's side -- the guard means
-- the fixed function is never asked.
--
-- FOUND BY SWEEPING. `and v_base.epf_wage > 0` -> `>= 0` survived one
-- hundred and forty-five mutants' worth of the payroll suite. Chasing
-- why it survived is what turned it up: the mutant is the fix.
--
-- The guards go, the eligibility checks stay. An employee genuinely
-- outside EPF consulted no table and names none; an employee inside it
-- who earned nothing this month names the table they would have
-- contributed under, and inherits its honesty. The amounts do not move
-- -- `calc_statutory` returns zero on a zero wage either way, which is
-- what `payroll_engine_sweep.sql` asserts alongside the flag.
...
begin
  ...
  loop
    ...
    if v_emp.eis_eligible and v_age < 60 then
      select c.employee_amount, c.employer_amount, c.schedule_id, c.is_verified
