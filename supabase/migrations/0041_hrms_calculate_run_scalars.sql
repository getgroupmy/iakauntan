-- =====================================================================
-- iAkauntan :: 0041 hrms calculate run scalars
-- Applied as project migration 20260810055813.
-- =====================================================================

-- Statutory results are held in plain scalars rather than record
-- variables: a record assigned from row() carries no field names, which
-- broke as soon as an employee was exempt from one of the schemes.
  ...
  loop
    ...
    if v_emp.socso_eligible and v_base.socso_wage > 0 then
      select c.employee_amount, c.employer_amount, c.schedule_id, c.is_verified
        ...
        from app.calc_statutory('socso',
               case when v_age >= 60 then 'act800' else 'act4' end,
