-- =====================================================================
-- Reversing it has to be enough
--
-- `import_opening_balances` refuses a second run with:
--
...
--
-- Reversing it did not help. `reverse_gl_entry` writes a second,
-- opposite entry and leaves the first one posted -- which is right, and
-- is how every reversal in this ledger works -- so the lookup behind
-- that message still found the original and refused again. The advice
-- was impossible to follow, and the only way out was a hand-written
-- update to gl_entries.
--
-- Found by a mutation sweep. The mutant was "the second-run check
-- ignoring whether the entry is posted": cut `and ge.status = 'posted'`
-- and nothing failed, because no test had ever got as far as reversing
-- one. Writing that test is what showed the message was wrong rather
-- than the check.
--
-- An entry that has been reversed is not an opening balance the company
-- is carrying. It no longer counts -- and neither does the reversal,
-- which copies the source and the source_table of the entry it undoes
-- and would otherwise be found in its place.
-- =====================================================================
create or replace function public.import_opening_balances(p_org_id uuid, p_rows jsonb, p_as_at date, p_commit boolean DEFAULT false)
...
begin
  ...
  if p_commit then
    ...
    -- 0525. "Reverse that entry before bringing in another" is what this
    -- refusal tells the operator to do, and until now doing it did not
    -- help: `reverse_gl_entry` writes a SECOND entry and leaves the
    -- first one posted, so the lookup still found it and the advice was
    -- impossible to follow. An entry that has been reversed is not an
    -- opening balance the company is carrying, so it does not count.
    if exists (select 1 from public.gl_entries ge
                where ge.org_id = p_org_id
                  ...
                  -- Not the reversal itself, which carries the same
                  -- source and source_table as the entry it undoes.
...
revoke all on function public.import_opening_balances(uuid, jsonb, date, boolean) from public, anon;
grant execute on function public.import_opening_balances(uuid, jsonb, date, boolean) to authenticated;
