-- =====================================================================
-- iAkauntan :: 0613 a link instead of a password
--
-- README: "Sign-in with anything other than a password: no OAuth, no
-- magic link, no two-factor."
--
-- The first clause is out of date -- `0579` built passkeys, which are
-- stronger than all three and are already in the console, on the web
-- and in Settings. What it is right about is the magic link, and that
-- is what this is.
--
-- ---------------------------------------------------------------------
-- Who it is for
...
-- already solved that. It is for the person who signs in twice a year
-- -- a director approving one resolution, an auditor looking at one
-- year -- and who will have forgotten the password by then, ask for a
-- reset, and end up in the same inbox anyway. A magic link is the reset
-- without the intervening screen.
--
-- ---------------------------------------------------------------------
-- It ships OFF, like the passkey and unlike everything else on this page
--
-- GoTrue sends the mail, not this product. Until an SMTP sender is
-- configured in the Supabase dashboard, the project falls back to a
-- rate that is deliberately unusable -- a handful of messages an hour,
-- shared across every address -- and the button quietly does nothing
-- for almost everybody who presses it.
--
-- A button that fails silently is worse than no button, because the
-- person who pressed it waits. So: dashboard first, then here. Same
-- order and same reason as `signin_show_passkey`, and
-- `docs/magic-link.md` is the list.
--
-- Not a security control either way. This decides whether the button is
-- drawn; GoTrue decides whether a link is issued and honoured.
-- =====================================================================

alter table public.landing_page
  add column if not exists signin_show_magic_link boolean not null default false;

comment on column public.landing_page.signin_show_magic_link is
  'Draw "Email me a link instead" on the sign-in form. Ships FALSE, '
  'like `signin_show_passkey` and unlike every other switch on this '
  'page: GoTrue sends the mail, and until an SMTP sender is configured '
  'in the Supabase dashboard the project''s fallback rate is '
  'deliberately unusable -- so the button would fail silently for '
  'almost everybody. Dashboard first, then here. Not a security '
  'control: it decides whether the button is drawn, and GoTrue decides '
  'whether a link is issued.';

-- ---------------------------------------------------------------------
-- The payload carries it to the sign-in screen
--
-- Restated whole from what is built, with one line added beside the
-- passkey's. That screen draws before anything is published, so the
-- switch has to reach it through `brand` rather than through the
-- marketing site.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- And the console learns to write it
--
-- Restated whole from what is built, with one clause added beside the
-- passkey's.
-- ---------------------------------------------------------------------
