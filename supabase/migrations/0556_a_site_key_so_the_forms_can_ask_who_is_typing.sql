-- =====================================================================
-- iAkauntan :: 0556 a site key, so the forms can ask who is typing
--
-- Asked for: a captcha on registration, sign-in, the forgot-password
-- link and the confirmation resend. Supabase Auth does the verifying
-- itself -- Authentication -> Attack Protection, where the provider's
-- SECRET goes -- and what the app has to do is show the widget and send
-- the token it produces.
--
-- Cloudflare Turnstile, chosen over hCaptcha: no puzzle for most
-- visitors, and the lightest thing to embed on the web, which is where
-- this product is actually used.
--
-- ---------------------------------------------------------------------
-- The site key is not a secret, and the secret is not here
--
-- Turnstile has two halves. The SITE key identifies the widget to the
-- browser; it is compiled into every page that draws one and proves
-- nothing on its own. The SECRET verifies a token server-side, and for
-- this product that server is GoTrue -- so the secret goes in the
-- Supabase dashboard and never in this repository, this table, or any
-- payload the app can read.
--
-- So the site key rides with the rest of the sign-in page's appearance,
-- on `landing_page`, editable in the console and readable by
-- `landing_page()`, which is already open to an unauthenticated caller
-- because that screen has nobody signed in.
--
-- ---------------------------------------------------------------------
-- Empty means off, and that ordering matters
--
-- With no site key the forms draw no widget and send no token, which is
-- exactly what they do today. That makes the switch-on order safe:
-- paste the key here first, watch a form draw the widget, and only
-- then turn the protection on in the dashboard. Doing it the other way
-- round refuses every sign-in on the project until the key arrives --
-- including yours.
--
-- THE ORDER MATTERS THE OTHER WAY TOO, and it is a trap worth naming:
-- the dashboard switch protects the whole project, not one platform.
-- The Android and iOS builds have no Turnstile widget in them, so
-- until one exists there, enabling the protection would lock every
-- phone out of sign-in. `docs/captcha.md` says so where somebody about
-- to press it will read it.
-- =====================================================================

alter table public.landing_page
  add column if not exists turnstile_site_key text;

comment on column public.landing_page.turnstile_site_key is
  'Cloudflare Turnstile SITE key, which is public by design. The '
  'secret that verifies a token lives in the Supabase dashboard. '
  'Empty means no captcha. 0556.';

-- ---------------------------------------------------------------------
-- Both restated from their built definitions, because between them
-- they carry sixty columns and rebuilding either from an older
-- migration would silently drop whatever arrived since.
-- ---------------------------------------------------------------------
