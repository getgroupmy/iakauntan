-- =====================================================================
-- iAkauntan :: 0033 hrms post payroll run
-- Applied as project migration 20260810055024.
-- =====================================================================

-- Post a calculated run to the ledger and roll the year-to-date figures
-- forward. Employer contributions are a cost of employment, so they are
-- expensed; every statutory amount lands in its own payable until it is
-- remitted.
...
begin
  ...
  if v_run.status <> 'calculated' and v_run.status <> 'approved' then
  ...
  -- Year to date, which the next month's PCB projection reads.
  ...
  -- Claims reimbursed with this payroll are now settled.
