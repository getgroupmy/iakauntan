-- ---------------------------------------------------------------------
-- 0335 - the demo logins are a decision, not a default
--
-- The sign-in screen offers a row of one-tap demo logins. They are
-- gated on `DEMO_MODE`, a compile-time flag that is off unless a build
-- asks for it - and this build asks for it, because the repository
-- variable is set. So the list is on everywhere, all the time, and the
-- only way to take it off is a rebuild.
--
-- That is the wrong grain. Whether to show strangers a way into a
-- seeded demo company is a decision about what the platform is doing
-- this week, and the person who makes it is looking at the console, not
-- at a repository variable in a CI configuration.
--
-- ## Two gates, and both have to be open
--
-- The compile-time flag stays exactly as it is, and it stays the outer
-- one. `demo_accounts.dart` says why, and it is still true: the panel
-- ships the demo password inside the bundle, so a build that did not
-- ask for demo mode should not be carrying it at all. A row in a table
-- cannot un-ship a password that is already in the JavaScript.
--
-- What this adds is the inner gate: `demo_accounts_enabled`, default
-- **false**, so a build that carries the flag still shows nothing until
-- a platform administrator turns it on. Off by default is the whole
-- point - the previous arrangement's failure was that forgetting to
-- set the flag and choosing to set it were the same act.
--
-- ## In `brand`, not in `page`
--
-- Same reason as `0331`'s four columns. `page` is gated on publication;
-- the sign-in screen draws whether or not anybody has published a
-- marketing site, and a switch that only takes effect once the front
-- page is live is a switch that does not work.
--
-- Both functions are recreated whole, copied from `0331`, because
-- `create or replace` has no other form - and re-granted below, because
-- `0165`'s event trigger strips privileges from anything replaced in
-- `public` or `app`.
-- ---------------------------------------------------------------------

alter table public.landing_page
  add column if not exists demo_accounts_enabled boolean not null default false;

comment on column public.landing_page.demo_accounts_enabled is
  'Offer the one-tap demo logins on the sign-in screen. Off by default, '
  'and only effective in a build made with --dart-define=DEMO_MODE=true.';
