-- =====================================================================
-- iAkauntan :: 0370 the levy charged on the wrong wage
--
-- `salary_components.is_hrdf_liable` has been a column since 0028,
-- sitting in a row with `is_epf_liable`, `is_socso_liable` and
-- `is_eis_liable`, and unlike the other three it has never been read by
-- anything. 0043 charges the HRD Corp levy on the EPF wage:
--
--     v_hrdf := round(v_base.epf_wage * v_hrdf_rate / 100, 2);
--
-- Those are not the same wage, and the difference is a statutory one.
--
-- ---------------------------------------------------------------------
-- What the levy is charged on
--
-- The Pembangunan Sumber Manusia Berhad Act 2001 defines wages for the
-- levy as basic salary plus fixed allowances. HRD Corp's own guidance
-- lists what that leaves out: overtime, commission, bonus and other
-- incentive payments, service charge, travelling allowance, gratuity,
-- and any payment on retirement, retrenchment or termination.
--
-- EPF is payable on most of those. A bonus is EPF wage and is not levy
-- wage, and in Malaysia a bonus is typically one or two months' salary
-- paid once a year — so a company with fifty people on RM 4,000 paying a
-- two-month bonus over-remitted about RM 4,000 of levy in that month, at
-- one per cent, on wages the Act says are not wages. Every other month
-- of the year agreed to the ringgit, which is what made it invisible.
--
-- Overtime was already outside the figure, but by accident rather than
-- by decision: EPF is not payable on overtime, so the EPF wage happened
-- to exclude it. Standing on that is standing on the wrong statute.
--
-- ---------------------------------------------------------------------
-- The flag reaches the payslip line
--
-- `payslip_lines` carries the other three flags because the wage each
-- authority charges on is a sum over the lines that are liable to it.
-- The fourth flag goes on beside them and the levy gets its own wage,
-- which means the arithmetic is now visible on the payslip rather than
-- inferred: `payslips.hrdf_wage` is the base, `hrdf` is the levy, and a
-- company that disagrees with a HRD Corp statement can see which of the
-- two it disagrees about.
--
-- ---------------------------------------------------------------------
-- What happens to the payslips already issued
--
-- The backfill sets `is_hrdf_liable` to `is_epf_liable`, not to the
-- correct value. A payslip is a record of what was paid and what was
-- remitted, and restating it would leave the stored levy disagreeing
-- with the stored base — a payslip that no longer explains itself. So
-- the history says what was actually done: the EPF wage was treated as
-- the levy wage. Anyone who wants the difference for a past month can
-- now compute it, which is the point.
--
-- A draft or calculated run recalculated after this lands gets the new
-- figure; an approved or posted one is untouched, as always.
-- =====================================================================

alter table public.payslip_lines
  add column if not exists is_hrdf_liable boolean not null default true;

-- What was actually charged, recorded as what was actually charged.
update public.payslip_lines set is_hrdf_liable = is_epf_liable
 where is_hrdf_liable is distinct from is_epf_liable;

alter table public.payslips
  add column if not exists hrdf_wage numeric(18, 2) not null default 0;

-- And for payslips issued before this, the base the levy was taken on.
update public.payslips set hrdf_wage = epf_wage
 where hrdf_wage = 0 and hrdf > 0;

comment on column public.payslip_lines.is_hrdf_liable is
  'Whether this line is wages for the HRD Corp levy. Narrower than EPF: '
  'the PSMB Act 2001 counts basic salary and fixed allowances, and '
  'excludes overtime, commission, bonus, service charge, travelling '
  'allowance and payments on termination.';
comment on column public.payslips.hrdf_wage is
  'The wage the HRD Corp levy was charged on, which is not the EPF wage '
  'in any month carrying a bonus.';
