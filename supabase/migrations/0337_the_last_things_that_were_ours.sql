-- ---------------------------------------------------------------------
-- 0337 - the last things on the sign-in page that were ours
--
-- `0336` gave the panel beside the form a switch and took our headline
-- and our three bullets out of the Dart. It left four things behind,
-- and all four are still this product's rather than the operator's:
--
--   * **the colour.** The panel is `scheme.primary`, which comes from
--     `brand_colour` and is therefore already an operator's choice -
--     but only the *same* choice as everything else. A platform whose
--     product is dark blue may well want a different colour on the one
--     screen that is half flat colour, and today it cannot say so.
--   * **the icon.** With no logo uploaded the mark falls back to a
--     compiled-in wallet, which is iAkauntan's mark drawn on somebody
--     else's sign-in page. It is not a placeholder - a visitor cannot
--     tell it from a real logo.
--   * **the name.** `?? 'iAkauntan'` appears three times in the screen.
--     `landing_page.wordmark` is NOT NULL and always answers, so those
--     literals never ran except against an empty payload - which is
--     exactly the case where inventing a name is worst.
--   * **the words on the form.** Email, Password, Full name, Forgot
--     password?, and the two sentences offering an account.
--
-- ## And two columns that already existed and were being ignored
--
-- `sign_in_label` and `register_label` have been on this table since
-- `0290`. The landing page reads them; the sign-in screen never did. So
-- an operator who renamed the button watched it change on the front
-- page and stay the same on the form the button leads to, which reads
-- as a bug in the console rather than a gap in the screen. They go into
-- `brand` here so the screen can read them too.
--
-- ## Nullable, and null means the word we ship
--
-- Unlike `0336`'s switches, none of this defaults to off or to empty. A
-- form whose fields have no labels is not a cleaner form, it is a
-- broken one - so the default is the word the product ships with, and
-- clearing a box in the console asks for that word back.
--
-- `signin_panel_colour` is the same shape: null means the brand colour,
-- which is what the panel is today.
--
-- ## What is deliberately not here
--
-- The validation messages ("Enter your email", "Use at least 8
-- characters"), the notices, and whatever GoTrue returns when a
-- password is refused. Those are not branding - they are what the form
-- says when it cannot do what was asked, they change with the rules
-- rather than with the platform, and a console field for each would be
-- twenty rows nobody edits standing between an operator and the six
-- that matter.
--
-- The status palette in `theme.dart` is not here either. Green for
-- money in and red for a refusal are meanings rather than decoration,
-- and a platform that recoloured them would be lying to its own users.
-- ---------------------------------------------------------------------

alter table public.landing_page
  add column if not exists signin_panel_colour    text,
  add column if not exists signin_email_label     text,
  add column if not exists signin_password_label  text,
  add column if not exists signin_name_label      text,
  add column if not exists signin_forgot_label    text,
  add column if not exists signin_register_prompt text,
  add column if not exists signin_signin_prompt   text;

-- Same shape as `brand_colour`'s, and checked in the database as well
-- as in the saver: the saver is the door everybody uses, and the
...
alter table public.landing_page
  add constraint landing_page_signin_panel_colour_hex
    check (signin_panel_colour is null
           or signin_panel_colour ~ '^#[0-9A-Fa-f]{6}$');

comment on column public.landing_page.signin_panel_colour is
  'The colour of the panel beside the sign-in form. Null uses the brand '
  'colour, which is what it was before this column existed.';
comment on column public.landing_page.signin_email_label is
  'Label on the email box. Null uses the word the product ships with.';
comment on column public.landing_page.signin_password_label is
  'Label on the password box.';
comment on column public.landing_page.signin_name_label is
  'Label on the name box, which only the sign-up form draws.';
comment on column public.landing_page.signin_forgot_label is
  'The link under the password box.';
comment on column public.landing_page.signin_register_prompt is
  'The sentence offering an account, under the button.';
comment on column public.landing_page.signin_signin_prompt is
  'The sentence back to signing in, shown on the sign-up form.';
  ...
  select jsonb_build_object(
    ...
    'brand', coalesce((
      select jsonb_build_object(
               ...
               -- published.
               'signin_panel_colour',    p.signin_panel_colour,
...
begin
  ...
  update public.landing_page p set
    ...
    -- they just cleared.
    signin_panel_colour = case when p_patch ? 'signin_panel_colour'
                            then nullif(btrim(p_patch ->> 'signin_panel_colour'), '')
                            else p.signin_panel_colour end,
