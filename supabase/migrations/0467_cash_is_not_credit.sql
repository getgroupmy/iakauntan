-- ---------------------------------------------------------------------
-- 0467  Cash is not credit
-- ---------------------------------------------------------------------
-- `app.enforce_credit_limit` refuses to post an invoice for a customer
-- on credit hold, and tells whoever hit the problem:
--
--   "Encik Zaid is on credit hold, so INV-2026-00001 cannot be posted.
--    Take the hold off, or raise this as a cash sale."
--
-- Measured: that message is what the **till** prints when a customer on
-- hold walks in and pays cash. The sale is rung up, the drawer is open,
-- the money is on the counter, and the advice is to do the thing that
-- has just been done. There is no way forward from that screen except
-- taking the hold off the customer, which is a decision about credit
-- being forced by somebody buying a coffee.
--
-- The same is true of the limit: a customer at their limit cannot spend
-- cash at the counter either.
--
-- ### What a credit limit is for
--
-- Credit. A sale settled at the point of sale extends none, and a limit
-- that refuses it is not protecting anybody from anything -- the money
-- is already in the drawer.
--
-- But a counter sale is not always cash: `complete_pos_sale` has an
-- on-account tender, and its own comment says so -- "an on-account
-- tender is a promise, so the receipt is for the rest of the basket and
-- the remainder stays on the invoice for the customer to settle later".
-- That promise *is* credit, and it is the part a limit has business
-- refusing.
--
-- So the rule is not "counter sales are exempt". It is **the credit
-- being extended is what is left on account**, which for an ordinary
-- invoice is the whole document and for a counter sale is the
-- on-account tender: nil for the customer paying cash, the balance for
-- the customer who asks the shop to put it on their tab.
--
-- ### One statement had to move
--
-- The trigger fires while the invoice is being posted and asks
-- `pos_sales` by `invoice_id` how much of this sale is on account.
-- `complete_pos_sale` wrote that link in the statement that finishes
-- the sale, which runs afterwards -- so the answer was not there when
-- the question was asked. The link and the on-account figure are now
-- written just before the posting, and the completing statement still
-- runs unchanged below.
--
-- ### Mutants
--
-- Four, restated into a built database and run against
-- `supabase/tests/cash_is_not_credit.sql`. All four die; **one survived
-- the assertions as first written**.
--
--   * the counter-sale exemption removed -- killed by "a customer on
--     hold can still buy a coffee with cash", which came back with the
--     measured refusal word for word;
--   * every counter sale exempted, on account or not -- killed by "the
--     shop still cannot put it on their tab". This is the assertion
--     that keeps the fix from being a hole: a tab is credit, and a hold
--     is precisely the instruction not to give this customer one;
--   * the credit measured against the basket rather than the tab --
--     **survived**. Every sale in the file was either all cash or all
--     on account, so the basket and the amount being lent were the same
--     number and the two readings could not be told apart. The file now
--     rings up a RM100 basket as RM80 cash and RM20 on account, against
--     RM500 owed and a RM550 limit: the tab reading allows it at RM520,
--     the basket reading refuses at RM600, and the mutant says
--     "would take Encik Zaid to 600.00";
--   * the `pos_sales` link written after the posting instead of before,
--     which is where it used to be -- killed by the first assertion.
--     The trigger's lookup simply finds nothing and every counter sale
--     goes back to being full credit, with no error anywhere to say so.
--     That is the failure this reordering exists to prevent, and it is
--     why the apply-time guard checks the order of the two statements
--     rather than merely that both are present.
-- ---------------------------------------------------------------------

-- ---------------------------------------------------------------------
-- The sale, with its invoice linked before it is posted
-- ---------------------------------------------------------------------
-- Restated from the live definition; the only change is the update
-- moved above `post_sales_document_internal`.
...
-- ---------------------------------------------------------------------
-- The rule, against what is actually being lent
-- ---------------------------------------------------------------------
...
declare
  ...
  v_name        text;
  v_rate        numeric(18, 8) := coalesce(new.exchange_rate, 1);
  v_onacct      numeric(18, 2);
  v_extending   numeric(18, 2);
begin
  ...
  -- nothing more, not pretend they owe nothing. See 0466.
  ...
  -- How much credit this document is actually extending. For an
  -- ordinary invoice it is the whole thing. For a counter sale it is
  -- the on-account tender and nothing else: the rest is in the drawer.
  -- See 0467.
  v_extending := coalesce(new.balance_amount, 0) * v_rate;

  select s.on_account_amount into v_onacct
    from public.pos_sales s where s.invoice_id = new.id;

  if found then
    if coalesce(v_onacct, 0) <= 0 then
      -- Paid at the till. Nothing has been lent, so there is nothing
      -- for a limit or a hold to refuse.
      return new;
    end if;
    v_extending := round(v_onacct, 2) * v_rate;
  end if;
  ...
  v_outstanding := v_outstanding + v_extending;
...
comment on function app.enforce_credit_limit() is
  'Refuses to lend more to a customer on hold, or past their limit when '
  'credit control is set to block. What counts as lending: the whole '
  'invoice, except that an opening balance is debt already owed (0466) '
  'and a counter sale lends only its on-account tender (0467). See 0057.';
...
do $do$
declare
  v_cl  text := pg_get_functiondef(to_regprocedure('app.enforce_credit_limit()'));
  v_pos text := pg_get_functiondef(to_regprocedure(
    'public.complete_pos_sale(uuid, jsonb, uuid)'));
begin
  if position('public.pos_sales s where s.invoice_id = new.id' in v_cl) = 0 then
    raise exception
      '0467: the till still refuses a customer who is paying cash';
  end if;

  -- The link has to be written before the posting or the lookup above
  -- finds nothing and every counter sale is treated as full credit
  -- again -- silently, because the trigger would simply not `found`.
  if position('set invoice_id = v_inv, on_account_amount = v_onacct'
              in v_pos)
     > position('perform app.post_sales_document_internal(v_inv)' in v_pos) then
    raise exception
      '0467: the sale is linked to its invoice after the posting, so the '
      'credit check cannot see how much was tendered on account';
  end if;

  -- 0466 is still in there.
  if position('''opening_balance''' in v_cl) = 0 then
    raise exception '0467: restating the trigger dropped 0466''s exemption';
  end if;
  if position('is on credit hold' in v_cl) = 0
     or position('against a credit limit of' in v_cl) = 0 then
    raise exception '0467: restating the trigger dropped 0057''s rule';
  end if;
end
$do$;
